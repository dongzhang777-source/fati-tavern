...
安全与数据可靠性专项。**含一处兼容性断裂，见文末。**

### Fixed

- **P2P 群聊密钥来源不再可伪造**：`group_key` 帧验签由「帧带签名才验」改为**无条件验签（fail-closed）**——缺少签名、票据内无算力端公钥、签名不匹配三种情况一律拒绝建立加密会话。此前中转方只要在转发时**删掉 `sig` 字段**，就能绕过这唯一能证明「密钥来自算力端」的防线，用自己替换的公钥重新加密群组密钥骗过客户端。
- **本地数据写/删失败不再静默**：角色、会话、世界书、剧情四类持久化操作，在 IndexedDB 写入或删除失败时（配额耗尽 / 隐私模式 / Safari 存储驱逐 / 事务冲突）统一给出四语错误提示，并保持**内存与库一致**——写入失败不塞进内存列表、删除失败不移出列表。此前这些失败会静默产生 unhandled rejection，表现为「界面显示成功、刷新后东西没了」，且用户拿不到任何提示。共覆盖主 store 10 处 + 世界书与剧情两个 slice 6 处写/删点。
- **批量导入成功数不再虚高**：角色与世界书的导入均改为按 store 返回值计数，写库失败的条目进入失败清单并说明原因，不再被计进成功数、也不再让界面 toast 虚高。
- **读失败不再白屏或点击无反应**：启动时角色库读取失败降级为仅显示内置角色并提示原因；点击角色读取会话失败则留在当前视图并提示，而不是「点了没反应」。
- **流式发送不再被重复触发打断**：进行中的流式请求会忽略再次发送，避免并发流式互相打断导致消息错序。

### Changed

- **P2P 加密说明订正**：修正文档中「中转服务器无法窃听」的过度承诺。ECDH 本身不防御主动中间人，保密性依赖两个前提——票据须经带外可信渠道传递、接收端不得回退到 fail-open；此外中转方仍可拒绝服务，文档已写明。

### Security

- 构建期依赖 vitest 4.1.10 → 4.1.11（GHSA-82fw-gwwq-j7x9）。该依赖不进入浏览器产物，无运行时影响。

### ⚠️ 兼容性断裂

- **请搭配 fati 桌面端 v0.54.0 或更高版本使用。** 本版把 `group_key` 帧签名收为强制校验，**不带签名的旧版 fati 算力端会被拒绝**，表现为 P2P 群聊无法建立加密会话。若你同时使用 fati 桌面端发起群聊，请先升级桌面端到 v0.54.0+。浏览器端（本应用）会自动更新，无从选择，这也是本次必须写进发版说明的原因。
