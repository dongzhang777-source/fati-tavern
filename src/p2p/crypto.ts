...
//
// ⚠️ 对 relay 的保密性承诺（2026-09-11 TV-01 后订正，勿回退为无条件表述）：
//   单纯 ECDH **不足以**防主动 MITM——公钥在第 1/2 步明文经 relay 转发且本身无签名，
//   主动 relay 可替换双边公钥、以自造共享密钥重加密群组密钥。该缺陷已于 2026-09-11
//   由 TV-01 修复：算力端对 group_key 帧做 Ed25519 签名，本端用**票内 pk** 验签并改
//   fail-closed（见 `client.ts`：无 sig / 票内无公钥 / 验签失败 三拒）。
//   故成立的是**有前提的**承诺：
//     ✅ relay 无法解读群聊内容（它替换 ECDH 公钥后伪造的帧验签必失败）；
//     ⚠️ 前提：票内 pk 即算法力端公钥——依赖**邀请票经可信带外通道**交给用户
//        （票由算力端自签，属票内自验签，无外部锚点）；
//     ⚠️ 前提：验签侧不得回退 fail-open（回归测试
//        `src/p2p/__tests__/groupkey-signature.test.ts` 经变异测试钉住）。
//   relay 仍可**拒绝服务**（丢帧/断连）——保密性不覆盖可用性。
//
// ⚠️ 兼容性：本端已 fail-closed，**不再接受**未带签名的旧算力端所发 group_key 帧。
//   「旧 fati + 新 tavern」不兼容，属本次修复的预期代价。
