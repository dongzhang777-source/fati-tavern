...
const dict: Record<string, { zh: string; en: string; ja: string; ko: string }> = {
  ...
  'landing.privacyBody': {
    zh: '你的 API Key、角色卡、聊天记录全部只存在你的浏览器中。没有后端服务器，没有内容上传，没有账号注册；主动分享时，卡片数据只保存在链接 # 后面。我们仅统计匿名的功能使用计数（访问 / 导入 / 发消息 / 角色编辑 / 嘴替使用 / 二轮对话 / 分享创建 / 分享打开），不含任何聊天内容、角色卡或 Key。',
    en: 'Your API key, character cards and chats live only in your browser. No backend, no content uploads, no sign-up; when you choose to share, card data stays only after the link’s #. We only collect anonymous feature-usage counts (visit / import / message / character edit / reply helper / second round / share created / share opened) — never any chat content, cards or keys.',
    ja: 'あなたのAPIキー、キャラクターカード、チャットはすべてブラウザ内にだけ保存されます。バックエンドサーバーも、コンテンツのアップロードも、アカウント登録もありません。共有を選択した場合、カードデータはリンクの#以降だけに保存されます。当サイトは匿名の機能利用回数（訪問／インポート／送信／キャラクター編集／返信アシスト／2回目の対話／共有作成／共有オープン）のみを集計し、チャット内容・カード・キーは一切収集しません。',
    ko: 'API 키, 캐릭터 카드, 채팅은 모두 브라우저에만 저장됩니다. 백엔드 서버도, 콘텐츠 업로드도, 회원가입도 없습니다. 공유를 선택하면 카드 데이터는 링크의 # 뒤에만 저장됩니다. 당사는 익명의 기능 사용 횟수(방문/가져오기/메시지/캐릭터 편집/답변 도우미/2차 대화/공유 생성/공유 열기)만 집계하며, 채팅 내용·카드·키는 절대 수집하지 않습니다.',
  },
