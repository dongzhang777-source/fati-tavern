...
const dict: Record<string, { zh: string; en: string; ja: string; ko: string }> = {
  ...
  'landing.privacyBody': {
    ...
    ja: 'あなたのAPIキー、キャラクターカード、チャットはすべてブラウザ内にだけ保存されます。バックエンドサーバーも、コンテンツのアップロードも、アカウント登録もありません。ページを閉じてもすべてローカルに残ります。当サイトは匿名の機能利用回数（訪問／インポート／メッセージ送信）のみを集計し、チャット内容・カード・キーは一切収集しません。',
    ko: 'API 키, 캐릭터 카드, 채팅은 모두 브라우저에만 저장됩니다. 백엔드 서버도, 콘텐츠 업로드도, 회원가입도 없습니다. 페이지를 닫아도 모든 것은 로컬에 남습니다. 당사는 익명의 기능 사용 횟수(방문/가져오기/메시지)만 집계하며, 채팅 내용·카드·키는 절대 수집하지 않습니다.',
  },
